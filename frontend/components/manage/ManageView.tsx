...
/** Index management: trigger /ingest, watch /status, browse /files. */
    ...
    <div className="min-h-screen bg-paper">
      <header className="border-b border-line bg-sidebar">
        <div className="mx-auto flex max-w-4xl items-center justify-between px-6 py-3">
          ...
          <Link
            href="/"
            className="rounded-lg px-3 py-1.5 text-sm text-ink-soft hover:bg-hover hover:text-ink"
          >
            Back to chat
          </Link>
        </div>
      ...
      <main className="mx-auto max-w-4xl px-6 py-8">
        <h1 className="text-2xl font-semibold tracking-normal">Index management</h1>
        ...
        <form
          ...
          className="mt-6 rounded-xl border border-line bg-panel p-5 shadow-[0_1px_4px_rgba(0,0,0,0.04)]"
        >
          <div className="flex flex-col gap-3 sm:flex-row">
            <label className="flex-1 text-sm">
              ...
              <input
                ...
                className="w-full rounded-lg border border-line bg-paper px-3 py-2 font-mono text-[13px] outline-none focus:border-line-strong"
              />
            </label>
            <label className="text-sm sm:w-48">
              ...
              <input
                ...
                placeholder="my-repo"
                className="w-full rounded-lg border border-line bg-paper px-3 py-2 text-[13px] outline-none focus:border-line-strong"
              />
            </label>
          </div>
          {formError !== null ? (
          ...
          <button
            ...
            className="mt-4 rounded-lg bg-ink px-4 py-2 text-sm font-medium text-panel disabled:opacity-40"
          >
            ...
            <div className="mt-3 text-sm">
              ...
              {status.last_successful_index ? (
                <p className="mt-2 border-t border-line pt-2 text-[13px] text-ink-soft">
                  ...
                  <b>{status.last_successful_index.repo_label}</b> -{" "}
...
function StateBadge({ state }: { state: StatusResponse["state"] }) {
  const styles: Record<StatusResponse["state"], string> = {
    ...
    running: "bg-user text-ink border-line-strong",
    completed: "bg-user text-ink border-line-strong",
